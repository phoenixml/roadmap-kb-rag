...
rows = json.loads(Path("outputs/qa_eval_small2.json").read_text(encoding="utf-8"))
def clean(p):
...
Path("outputs/bertscore_small2.json").write_text(json.dumps(out,indent=2),encoding="utf-8")
print("[OK] saved outputs/bertscore_small2.json", flush=True)
